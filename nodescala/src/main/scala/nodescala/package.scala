...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      val promises = Promise[List[T]]
      fs.foldLeft(promises.success(Nil).future) {
        (list, arg) => for {headList <- list; tailElem <- arg} yield headList :+ tailElem
      }
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val cancellationToken: CancellationTokenSource = CancellationTokenSource()
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val promise = Promise[S]
      f.onComplete(arg => promise.success(cont(arg)))
      promise.future
    }
